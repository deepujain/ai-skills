...
- **Classify Jenkins pipeline timeouts from the raw console.** When the console ends with `Caught signal TERM` and exit code `143` at the configured pipeline timeout while tests were still running, and it contains no deterministic patch-specific error, treat the red `This commit cannot be built` status as infrastructure timeout evidence. Push the least-invasive CI refresh, document the exact prior build and new head, and do not change source code solely for that status.
