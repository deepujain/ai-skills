...
4. **Commit message:** Has Changelog trailer and Signed-off-by; no tool attribution.
...
- **Message format:** Short subject; optional body; then trailers, e.g.:
  ...
  ```
- **Commit command (with sign-off and author):** Give the user the full command block to run in their terminal (they run git; do not run git for them). Use `--author` and `-s`; do not add coding-agent attribution.
- **Amend / no tool attribution:** If the host added tool attribution or the wrong author/Signed-off-by, give the user an **amend block** to run: `git commit --amend --no-verify --author="Deepak Jain <deepujain@gmail.com>" -m '...'` with the full message and Signed-off-by: Deepak Jain &lt;deepujain@gmail.com&gt;. Then regenerate the patch with `git format-patch -1 -o . master..HEAD`.
...
| Tool attribution or wrong author in patch | Amend the commit first (remove attribution; fix Signed-off-by to "Deepak Jain <deepujain@gmail.com>"), then regenerate with `git format-patch -1 -o . master..HEAD`. The patch contains the commit message, so formatting before amending preserves the bad line. |
...
- **Amend then format-patch.** If the host added tool attribution or the wrong author/Signed-off-by, give the user an **amend command block** first. Only after the commit message is clean should they run `git format-patch -1 -o . master..HEAD`; otherwise the attached patch still contains the bad trailer.
